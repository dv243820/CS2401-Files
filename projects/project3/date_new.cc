/*************************************************************************
    ...
    John Dolan			    September 2005    OHIO UNIVERSITY SCHOOL OF EECS
...
/* When passed value is current date, returns "age" of a date in years */
int Date::years_old(const Date &tday) const
{
    if (month < tday.month)
...
/* When passed value is current date, returns days old */
int Date::days_old(const Date &tday) const
{
    unsigned year_days = (tday.year - year) * 365.25;
    unsigned month_days = 0;
    unsigned day_days;
    if (tday.month == month)
    {
        day_days = tday.day - day;
    }
    else
    {
        day_days = day - 1;
    }
    for (int i = month; i < tday.month; i++)
    {
        month_days += daysallowed[i];
    }
    if (month > 2 && year % 4 != 0 && tday.year != year) // correction for non-leap years
        month_days -= 1;
